...
const readDataOnCache = async (url) => {
    ...
    // if you come from /sodoo to project the url is now /sodoo/project, but it doesn't exist in cache so use /sodoo instead
    if (!response) {
...
const serveShareTarget = (event) => {
    ...
    event.respondWith(Response.redirect("/sodoo?share_target=trigger"));
    event.waitUntil(
        (async () => {
